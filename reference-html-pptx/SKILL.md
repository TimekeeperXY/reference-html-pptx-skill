...
description: Build polished, logically structured 16:9 HTML presentation pages from a user-provided text-free background image, one or more design reference images, and slide copy, using reusable content-matched component recipes when appropriate, then visually verify and export them as structurally editable PPTX files. Use when the user asks to 仿照参考图做PPT、根据背景图和内容页示例生成HTML幻灯片、调用预置排版组件、优化PPT内容逻辑与版式、批量生成同风格多页演示、网页PPT转可编辑PPTX, or provides background/reference screenshots plus slide content. Do not use Dashi PPT.
---
...
Current design-workflow version: **2.7.0**.

Create slides with Codex's own content reasoning and HTML/CSS design ability. Optimize in this order:

**conclusion → semantic relationships → component matching → grouping → hierarchy → layout → wireframe → visual tokens → component instantiation → visual craft → PPTX annotation → group annotation → dual-render QA**
...
### 3.5 Match reusable components

When the content contains repeated peers or a recognizable information role, run:

```powershell
node scripts/select-components.mjs "<output>\design-plan.json"
```

The selector ranks recipes from `references/component-registry.json` using relation type, layout family, content signals, and peer count. Treat the result as a design shortlist, not an automatic page template. Choose a component only when its slots, capacity, and boundary language fit the content. Prefer one primary component family and no more than two supporting families per ordinary slide. Use the same component type and variant for parallel peers, and allow a custom composition when no registered recipe fits.
...
Use hierarchy, alignment, distance, whitespace, and background regions before adding borders or cards. Avoid card-per-sentence layouts, nested cards, duplicate borders, and decoration without information value. Keep accent color scarce and purposeful. When using a registered recipe, mark each instance with `data-component-type`, a unique `data-component-instance`, the chosen `data-component-variant`, and named `data-component-slot` elements. Mark informational cards with `data-density-card="true"`, the primary focus with `data-emphasis-level="primary"`, the principal content region with `data-slide-content`, and its major occupied blocks with `data-content-block`. Apply exemptions only with a documented semantic reason.
...
Report slide count, component family and instance counts, editable text-object count, editable shape-object count, editable line-object count, editable chart-object count, SVG image-object count, native PowerPoint group-object count, intentional raster-only count, unsupported semantic count, and structural editability coverage. Identify intentionally flattened semantic objects. Mention font substitution only when non-system fonts are used.
...
- Do not force a registered component when the content relationship or slot capacity does not fit; component reuse is a selection aid, not a mandate to make every page a grid.
